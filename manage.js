...
const editorOutlineList = document.querySelector("[data-manage-outline-list]");
const editorOutlineCount = document.querySelector("[data-manage-outline-count]");
const editorOutlineEmpty = document.querySelector("[data-manage-outline-empty]");
...
function showEditor(post) {
  if (!post) {
    ...
    if (editorOutlineList) editorOutlineList.innerHTML = "";
    if (editorOutlineCount) editorOutlineCount.textContent = "0 节";
    if (editorOutlineEmpty) editorOutlineEmpty.hidden = false;
    closeSlashMenu();
    return;
  }
...
document.addEventListener("click", (event) => {
  ...
  if (slashMenu && !slashMenu.hidden && !event.target.closest("[data-editor-command-menu]") && !event.target.closest("[data-manage-media-flow]")) {
    closeSlashMenu();
  }
});
